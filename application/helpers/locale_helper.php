...
function get_report_name($type, $report_id) {
    $title='';
    include 'application/language/en-us/global_lang.php';
    include 'application/language/en-us/report_lang.php';
    
    if ($type == ONLINE_REGISTERED_TYPE)
        $title= $lang["job_seekers_reports_online_registered"];
    else
        $title=$lang["job_seekers_reports_interviewed"];
    $list=  get_seekers_about_list_lines();
    $title.=' - '.$lang[$list[$report_id]];
    return $title;
}
